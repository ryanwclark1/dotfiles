...
configure_shell() {
  ...
  # Clear any conflicting starship environment variables from previous shell sessions
  # This fixes issues where starship was initialized for zsh but current session is bash
  ...
  # Reinitialize starship for the current shell to ensure it works immediately
  if command -v starship &>/dev/null; then
    case "$shell_type" in
      bash)
        eval "$(starship init bash)" 2>/dev/null || echo "Note: Starship initialization will take effect in new shell sessions"
        ;;
      zsh)
        eval "$(starship init zsh)" 2>/dev/null || echo "Note: Starship initialization will take effect in new shell sessions"
        ;;
    esac
  fi
}
